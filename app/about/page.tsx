...
export default function AboutPage() {
  return (
    <main className="about-page">
      ...
      <section className="pdf-archive-section"><div className="section-top"><div><div className="section-label">05 — PDF archive</div><h2>The archive<br /><i>in full.</i></h2></div><p className="section-note">All image plates from the supplied ArTree Nepal PDF, preserved as a visual record of the collective’s work and research.</p></div><div className="pdf-grid">{pdfArchiveImages.map((src, index) => <figure key={src}><Image unoptimized src={src} alt={`ArTree Nepal PDF archive image ${index + 1}`} fill loading="lazy" sizes="(max-width: 700px) 50vw, 16vw" /><figcaption>{String(index + 1).padStart(2, "0")}</figcaption></figure>)}</div></section>
