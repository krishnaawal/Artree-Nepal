...
export default function AboutPage() {
  ...
  useEffect(() => {
    function closeOnEscape(event: KeyboardEvent) { if (event.key === "Escape") setPreview(null); }
  ...
  return (
    <main className="about-page">
      <header className="site-header about-header">
        <a href="/" className="brand" aria-label="ArTree Nepal home"><Image className="logo logo-small" src="/images/site/artree-logo.png" alt="ArTree Nepal" width={42} height={42} /></a>
      ...
      {preview && <div className="image-lightbox" role="dialog" aria-modal="true" aria-label={`Preview of archive image ${preview.index + 1}`} onClick={() => setPreview(null)}><div className="lightbox-panel" onClick={(event) => event.stopPropagation()}><button className="lightbox-close" type="button" onClick={() => setPreview(null)} aria-label="Close image preview">×</button><div className="lightbox-image"><Image src={preview.src} alt={`Enlarged ArTree Nepal PDF archive image ${preview.index + 1}`} fill sizes="92vw" /></div><span>Archive image {String(preview.index + 1).padStart(2, "0")}</span></div></div>}
    </main>
  );
}
