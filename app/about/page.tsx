...
const visiblePdfArchiveImages = pdfArchiveImages.filter((_, index) => ![0, 1, 32, 33].includes(index));
const fieldImages = [
...
export default function AboutPage() {
  ...
  return (
    <main className="about-page">
      ...
      {preview && <div className="image-lightbox" role="dialog" aria-modal="true" aria-label={`Preview of archive image ${preview.index + 1}`} onClick={() => setPreview(null)}><div className="lightbox-panel" onClick={(event) => event.stopPropagation()}><button className="lightbox-close" type="button" onClick={() => setPreview(null)} aria-label="Close image preview">×</button><div className="lightbox-image"><Image unoptimized src={preview.src} alt={`Enlarged ArTree Nepal PDF archive image ${preview.index + 1}`} fill sizes="92vw" /></div><span>Archive image {String(preview.index + 1).padStart(2, "0")}</span></div></div>}
    </main>
  );
}
